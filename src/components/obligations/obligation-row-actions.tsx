...
import {
  ...
  deleteObligation,
  setObligationActive,
} from "@/services/obligations-service.client";
...
export default function ObligationRowActions({
  obligationId,
  wallets,
  showPay,
  isActive,
  ...
  const handlePay = async () => {
    ...
    setIsPaying(true);
    try {
      await payObligation(obligationId, walletId);
      router.refresh();
    } catch {
  ...
  const handleToggleActive = async () => {
    setIsToggling(true);
    try {
      await setObligationActive(obligationId, !isActive);
      router.refresh();
    } catch (error) {
      toast.error(
        error instanceof ApiError ? error.message : "No se pudo actualizar la obligación.",
      );
    } finally {
      setIsToggling(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error instanceof ApiError ? error.message : "No se pudo eliminar la obligación.");
    } finally {
      setIsDeleting(false);
    }
  };
